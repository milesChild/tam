...
class DatabaseApp(EClient, wrapper.EWrapper):
    ...
    def historicalDataRequest(self, contract: Contract) -> None:
        ...
        logging.info("Sleeping 20 seconds now")
        time.sleep(30)
...
def main(syms: List):
    ...
    argp.add_argument("--size", type=str, default="1 day", help="bar size")
    argp.add_argument("--duration", type=str, default="1 D", help="bar duration")
    argp.add_argument(
        "-t", "--data-type", type=str, default="BID", help="bar data type"
    )
    argp.add_argument(
        "--base-directory",
        type=str,
    ...
    argp.add_argument(
        "--security-type", type=str, default="BOND", help="security type for symbols"
    )
    argp.add_argument(
        "--start-date",
        help="First day for bars",
        default=now - timedelta(days=40),
        action=DateAction,
    )
    argp.add_argument(
    ...
    app = DatabaseApp(contracts, args)
...
if __name__ == "__main__":
    df = pd.read_csv('files/Agg_holdings.csv')
    cusips = [cusip for cusip in df[df['CUSIP'].notna()]['CUSIP'].tolist() if len(cusip)==9]
    main(cusips[50:60])